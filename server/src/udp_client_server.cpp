...
void printHex(int nbytes, char *buffer) {
	for (int i=0; i < nbytes; i++) {
		printf("%02X", buffer[i]);
	}
}

void printHex(int buffer) {
	printHex(sizeof(buffer), (char *) &buffer);
}
...
namespace udp_client_server
{
	...
	/*Wrapper for std::vector of uint32s */
	
	int udp_client::send(std::vector<uint32_t> data)
	{
		return send((char *) &data[0], data.size()*sizeof(data[0]));
	}
